...
- **Length**: 120–180 words is the soft target for a typical bill (matches `BillsTab.tsx`'s own displayed guidance). A genuinely complex or contested bill (e.g. multi-provision appropriations, real two-sided opposition) may honestly need well more than that — do not truncate and drop real mechanism/distributional information to hit the number. This is Criterion 9 overriding Criterion 7 by design (see the rubric above) — expect it on any non-trivial bill, not just an occasional outlier. A genuinely simple bill should be honestly short, not padded to look substantial.
