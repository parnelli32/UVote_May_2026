...
3. **Output format is fixed by today's shipped frontend, not by this skill's judgment.** `summary` must be the inline `1. ... 2. ... 3. ... 4. ...` format, exactly four non-empty sections, validated against the *real* parser (`scripts/format_gate.mjs`) before the bill is considered done. No 5th section, no markdown, no bullets, no headers — those are real future improvements (see "Explicitly out of scope") but require a frontend change this skill does not make. The one exception, now shipped (see Step 7's Length rule): an optional `[[READ MORE]]`-delimited expandable-detail suffix, rendered by `BillDetailPage.tsx`'s own expand/collapse toggle — this is not a 5th section and is split off before the four-part parser ever sees it.
...
**If positive:**
- Still draft the full four-part `summary`, `bill_text`, `topic`, and `short_description` exactly as normal — the finding adds information, it never replaces or skips the citizen-facing content. It stays captured, not discarded.
...
- **Length**: 120–180 words is the soft target for the **mandatory core** — the four-part `summary` up through Part 4 — for a typical bill (matches `BillsTab.tsx`'s own displayed guidance). The core alone must satisfy all 9 rubric criteria on its own; a citizen who reads only the core must still come away genuinely informed.
- **Expandable detail (`[[READ MORE]]`)**: for a genuinely complex or contested bill (multi-provision appropriations, itemized line items, extended sourcing/attribution, real two-sided opposition with more than one citable position) where the honest mechanism/distributional information doesn't fit the core's soft target, do not inflate the core to absorb it. Instead, end the four-part core, then append the literal marker `[[READ MORE]]` followed by the supplementary detail as plain prose (`BillDetailPage.tsx` renders this behind its own expand/collapse toggle, split off before the four-part parser ever sees it — see `splitSummaryCoreAndDetail` in `src/lib/billUtils.ts`). What belongs in the core versus in expandable detail follows Step 6's axes: the core mechanism (Part 1) and the single strongest counter-argument (Part 4, Axis A) always go in the core; itemized appropriations lines, the long tail of stakeholder detail, and extended source attribution belong in expandable detail. Most bills — anything genuinely simple — need no `[[READ MORE]]` suffix at all; do not add one just to pad a bill that doesn't need it (Criterion 9 again). Never place the marker inside a section's text or before Part 4 — it must come only after the complete, well-formed four-part core.
- For a genuinely simple bill with nothing that warrants expandable detail, a shorter-than-180-word core with no `[[READ MORE]]` suffix at all is the correct, honest output — do not manufacture a "More detail" toggle a citizen would tap into and find nothing of substance.
- If the core itself, on its own, is complex enough that dropping content to hit 180 words would cut real mechanism/distributional information (and moving that content to expandable detail genuinely isn't appropriate — e.g. it's core-level material, not supplementary), letting the core run over the soft target remains acceptable, same as before this convention existed. This is Criterion 9 overriding Criterion 7 by design (see the rubric above).
- **Avoid vote-tally phrasing that looks like a section marker**: never write a committee/floor vote as `8 to 3.` (or any `N. ` for N = 1–4) in the summary prose — `parseSummaryIntoSections`'s inline regex treats a bare `N. ` anywhere in the text as a Part-boundary marker, so `3.` gets misread as the start of Part 3 and silently truncates the real content after it. Write it as `8-3` (hyphen, no trailing period) instead. This exact bug was caught by the format gate in the pilot batch (`SB10`) and required a regeneration — catch it before Step 10, not after.
...
Core word count: <n> (target 120–180; note if intentionally outside range and why)
...
- **Any further frontend change** — a 5th "Sourcing & Confidence" section, markdown rendering, per-bill-type `BillCard` styling. Expand/collapse progressive disclosure (report `§19`–`§21` item 3) is now shipped (`BillDetailPage.tsx`'s `[[READ MORE]]`-driven toggle, see Step 7/Step 10 above) — the remaining three are real, recommended future improvements (see `report.md` §19–§22) but each needs its own scoped, separately-authorized change. A future "Sourcing & Confidence" 5th section is a direct beneficiary of the now-shipped expand/collapse structure — it would most naturally live as its own labeled block inside the expandable-detail area rather than as a real 5th `SECTION_LABELS` entry, avoiding the parser changes (`[1-4]` → `[1-5]`, both `.slice(0, 4)` caps) that a true 5th section would otherwise require — but building that is still out of scope here.
