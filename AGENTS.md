...
**`npm run typecheck` is currently red on `main`, app-wide, independent of any feature work.** The hand-maintained `Database` type in `src/lib/types.ts` only defines `Tables` (no `Views`/`Functions` keys), which the installed `@supabase/supabase-js`/`postgrest-js` version resolves to `never` row types instead of falling back permissively — every `.from(table).select(...)` call site errors with `Property 'x' does not exist on type 'never'`. `npm run build` (esbuild, no full type-check) and `npm run lint` (ESLint, a separate tool) are both unaffected and are reliable signals; `typecheck` is not, until someone either pins a compatible `supabase-js` version or fully models `Views`/`Functions` in `types.ts`.
...
**Data model** (`src/lib/types.ts`, hand-maintained Supabase `Database` type — not generated): `legislative_bodies` → `districts` → `representatives`, `bills` (with `bill_sponsors` join table), `users`, `user_votes`, `rep_votes`, `error_logs`, `voting_blocks`/`voting_block_members`. Migrations live in `supabase/migrations/` as timestamped SQL files (RLS policies are iterated on directly in migrations — several exist purely to add/fix/drop RLS policies).
