...
- **Voting Blocks: census-based default blocks.** Admin-created, join-code Voting Blocks have shipped (see `AGENTS.md`'s Architecture section) and already solve the cold-start problem for organizations with existing membership. Census-category system blocks (silent data collection and auto-enrollment by census answer, see `AGENTS.md`'s Census-category system blocks section) have also shipped. Still deferred: the visible block-browsing/discovery UI that would let citizens find and view these blocks.
